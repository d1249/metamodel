...
class CliOptions:
    ...
    skos_tags: bool
    mermaid_output: Optional[Path]


@dataclass(frozen=True)
class ConversionResult:
    """Artifacts produced during conversion."""

    owl_serialization: str
...
def parse_args(argv: Optional[Sequence[str]] = None) -> CliOptions:
    ...
    parser.add_argument(
        "--mermaid-output",
        help="Optional path to write a Mermaid diagram visualizing the metamodel.",
    )
    ...
    if args.mermaid_output:
        mermaid_output = None if args.mermaid_output == "-" else Path(args.mermaid_output)
...
def generate_mermaid_diagram(document: dict) -> str:
    """Create a Mermaid graph representation of the metamodel."""

    entities = document.get("entity_kinds") or []
    relations = document.get("relation_kinds") or []

    lines = [
        "%% Auto-generated by metamodel2owl",
        "graph LR",
        "    classDef entity fill:#e3f2fd,stroke:#1e88e5,stroke-width:1px;",
        "    classDef attribute fill:#ede7f6,stroke:#5e35b1,stroke-width:1px;",
    ]

    entity_nodes: Dict[str, str] = {}
    attribute_nodes: set[str] = set()

    def make_entity_node(entity_id: str, label: Optional[str] = None) -> str:
        node_id = entity_nodes.get(entity_id)
        if node_id:
            return node_id
        node_id = sanitize_mermaid_identifier(f"entity_{entity_id}")
        safe_label = escape_mermaid_label(label or entity_id)
        lines.append(f'    {node_id}["{safe_label}"]')
        lines.append(f"    class {node_id} entity;")
    ...
    for entity in sorted(entities, key=lambda e: e.get("id", "")):
        ...
        if not entity_id:
            continue
        node_id = make_entity_node(entity_id, entity.get("name"))
        attributes = entity.get("attributes") or []
        for attribute in sorted(attributes, key=lambda a: a.get("id", "")):
            attr_id = attribute.get("id")
            if not attr_id:
                continue
            mermaid_attr_id = sanitize_mermaid_identifier(f"attr_{attr_id}")
            if mermaid_attr_id in attribute_nodes:
                continue
            attribute_nodes.add(mermaid_attr_id)
            attr_name = attribute.get("name") or attr_id
            data_type = (attribute.get("properties") or {}).get("type")
            label = f"{attr_name}"
            if data_type:
                label = f"{label} : {data_type}"
            safe_label = escape_mermaid_label(label)
            lines.append(f'    {mermaid_attr_id}("{safe_label}")')
            lines.append(f"    class {mermaid_attr_id} attribute;")
            lines.append(f"    {node_id} --> {mermaid_attr_id}")

    for relation in sorted(relations, key=lambda r: r.get("id", "")):
        from_kind = relation.get("from_kind")
        to_kind = relation.get("to_kind")
        if not from_kind or not to_kind:
            continue
        from_node = make_entity_node(from_kind)
        to_node = make_entity_node(to_kind)
        label = relation.get("name") or relation.get("id") or ""
        label = escape_mermaid_label(label)
        if label:
...
def sanitize_mermaid_identifier(value: str) -> str:
...
def run(options: CliOptions) -> ConversionResult:
    document = load_yaml(options.input_path)
    validate_against_schema(document)
    graph = build_graph(document, options)
    serialized = serialize_graph(graph, options.fmt)
    mermaid_diagram = None
    if options.mermaid_output is not None:
        mermaid_diagram = generate_mermaid_diagram(document)
    return ConversionResult(serialized, mermaid_diagram)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    try:
        ...
        result = run(options)
        write_output(result.owl_serialization, options.output_path)
        if options.mermaid_output is not None:
            if result.mermaid_diagram is None:
                raise MetamodelConversionError("Mermaid diagram was not produced.")
            write_output(result.mermaid_diagram, options.mermaid_output)
        return 0
